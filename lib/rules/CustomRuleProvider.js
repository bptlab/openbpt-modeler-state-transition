...
export default class CustomRuleProvider extends RuleProvider {
  ...
  init() {
    ...
    // CustomModelerTodoDone: Define which elements may be the source of a connection.
    // Returns the type of connection to be used. May depend on the source element type.
    this.addRule("connection.start", function (context) {
      const source = context.source;
      return (
        is(source, `${MODELER_PREFIX}:Node`) && {
          type: `${MODELER_PREFIX}:Transistion`,
        }
      );
    });
    ...
    // CustomModelerTodoDone: Define which elements may be resized.
    this.addRule("shape.resize", function (context) {
      if (context.shape.type === `${MODELER_PREFIX}:State`) {
        return true;
      }
      return false;
    });
...
// CustomModelerTodoDone: Define logic to regulate which elements can be connected.
// Must return the type of the connection to be created.
function canConnect(source, target) {
  return (
    (source.type === `${MODELER_PREFIX}:State` &&
    target.type === `${MODELER_PREFIX}:State`)) &&
