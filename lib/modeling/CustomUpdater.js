...
export default class CustomUpdater extends CommandInterceptor {
  ...
  updateConnectionWaypoints = function (connection) {
    connection.businessObject.di.set(
      "waypoint",
      this._customElementFactory.createDiWaypoints(connection.waypoints),
    );
  };
  ...
  connectionWaypoints = function (source, target) {
    ...
    connection.waypoints = [
      center(connection.source),
      center(connection.target),
    ];
    connection.waypoints =
      this._connectionDocking.getCroppedWaypoints(connection);
